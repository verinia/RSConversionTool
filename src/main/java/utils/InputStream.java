...
public class InputStream extends Stream {
	...
	public int readIntV1() {
		return ((readByte() & 0xFF) << 8) + (readByte() & 0xFF) + ((readByte() & 0xFF) << 24)
				+ ((readByte() & 0xFF) << 16);
	}
	...
	public int readIntV2() {
		return ((readByte() & 0xFF) << 16) + ((readByte() & 0xFF) << 24) + (readByte() & 0xFF)
				+ ((readByte() & 0xFF) << 8);
	}
	...
	public String readString() {
		...
		return Miscellaneous.method2122(buffer, offset, length);
	}
